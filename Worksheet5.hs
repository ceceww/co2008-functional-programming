...
goLeft :: (Btree a, Path) -> (Btree a, Path) 
goLeft (Branch lt _, ps) = (lt, L:ps)  

goRight :: (Btree a, Path) -> (Btree a, Path)
goRight (Branch _ rt, ps) = (rt, R:ps)
